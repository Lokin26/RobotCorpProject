# +
*** Settings ***
Documentation     Balamurugan's Robot Building Project for Certification
...               Initial Setup
...               Tasks for inital and cleanup 
...               Library
...
Order robots from RobotSpareBin Industries Inc
    ...
    FOR    ${order}    IN    @{orders}
        ...
        Fill the form    ${order}
        Wait Until Keyword Succeeds    40s    1s    Robot Preview
        Wait Until Keyword Succeeds    40s    1s    Order Submit
        ${pdf}=    Store the receipt as a PDF file    ${order}[Order number]
        ${screenshot}=    Take a screenshot of the robot    ${order}[Order number]
        Embed the robot screenshot to the receipt PDF file    ${screenshot}    ${pdf}
        Go to order another robot
    END
    [Teardown]    RobotTeardown
...
Robot Preview
    ...
    Wait Until Element Is Visible    id:preview    5s

*** Keywords ***
Order Submit
    Click Element    id:order
    Wait Until Element Is Visible    id:order-completion    5s
